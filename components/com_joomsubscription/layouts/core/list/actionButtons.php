...
<div class="d-flex">
	<div>
		<img src="<?php echo JUri::root(TRUE); ?>/components/com_joomsubscription/images/arrow-turn-270-left.png" alt="Select and"
		     class="arrow"/>
		<button type="button" class="btn-submit btn btn-outline-primary" onclick="if (document.adminForm.boxchecked.value==0){alert('<?php echo JText::_('SELECTFIRST', TRUE) ?>');}else{Joomsubscription.submitbutton('<?php echo $single; ?>.edit');}">
			<?php echo JText::_('EEDIT'); ?>
		</button>

		<div class="btn-group m-0 p-0">
			<button type="button" class="btn btn-outline-success" onclick="if (document.adminForm.boxchecked.value==0){alert('<?php echo JText::_('SELECTFIRST', TRUE) ?>');}else{Joomsubscription.submitbutton('<?php echo $controller; ?>.publish');}">
				<?php echo JText::_('EPUBLISH'); ?>
			</button>

			<button type="button" class="btn-outline-secondary btn" onclick="if (document.adminForm.boxchecked.value==0){alert('<?php echo JText::_('SELECTFIRST', TRUE) ?>');}else{Joomsubscription.submitbutton('<?php echo $controller; ?>.unpublish');}">
