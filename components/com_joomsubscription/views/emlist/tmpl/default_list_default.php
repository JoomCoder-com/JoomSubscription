			...
			<td align="center" width="<?php echo $width; ?>%" class="well well-small">
				<h2>
					<?php if($item->is_donation): ?>
						<?php echo \Joomla\CMS\Language\Text::_('EMINIMUM'); ?>
					<?php endif; ?>
		...
		<?php foreach($plans AS $item):?>
			<td align="center" valign="top">
				...
				<?php if(!empty($item->mua)): ?>
					<div><small><?php echo \Joomla\CMS\Language\Text::sprintf('You may share this cubscription with %d users', $item->mua); ?></small></div>
				<?php endif; ?>
			</td>
		<?php endforeach; ?>
	</tr>
		...
		<?php foreach($plans AS $item):?>
			<td align="center" width="<?php echo $width; ?>%">
				...
				<?php elseif($item->price <= 0 && !$item->params->get('properties.terms')): ?>
					<a class="btn btn-success" href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_joomsubscription&task=empayment.send&sid='.$item->id); ?>"><?php echo \Joomla\CMS\Language\Text::_('EBTNGETNOW'); ?></a>
				<?php elseif(!$item->is_donation): ?>
					<a href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_joomsubscription&view=empayment&sid='.$item->id); ?>" class="btn btn-warning">
						<?php echo \Joomla\CMS\Language\Text::_('EBTNBUYNOW'); ?></a>
				<?php else: ?>
					<a href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_joomsubscription&view=empayment&sid='.$item->id); ?>" class="btn btn-primary">
						<?php echo \Joomla\CMS\Language\Text::_('EBTNDONATE'); ?></a>
				<?php endif; ?>
			</td>
		<?php endforeach; ?>
	</tr>
