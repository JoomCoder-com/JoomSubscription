...
Picker.Date.Range = new Class({
	...
	options: {
		getStartEndDate: function(input){
			var field = JSON.decode(input.get('data-field-options'));
			var field_val = $$("[name^=\""+field.name+"\"]");
			
			if(field.input == 'filter')
			{
				field_val = field_val[0].value.split(',');
			}
			else
			{
				Array.each(field_val, function (item, i)  {
					
					field_val[i] = item.value;
				});
			}
			return field_val.map(function(date , i){
				var part = field_val[i].split('-');
				if(field.type == 'day')
				{
					var parsed = new Date(part[0], (part[1] - 1), part[2], 0,0,0);
				}
				else
	...
	updateRangeSelection: function(){
		...
		var formattedFirst = start.format_php(this.options.format)
			formattedEnd = end.format_php(this.options.format);
