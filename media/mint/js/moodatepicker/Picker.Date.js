...
*/
	...
	getInputDate: function(input){
		this.date = new Date();
		if (!input) return;
		if(field = JSON.decode(input.get('data-field-options')))
		{
			if(field.type)
			{
				var part = input.get('value').split(' ');
				var data = part[0].split(field.delimiter);
				if(part[1] != null)
				{
				...
				if(field.type == 'day')
					var date = new Date(data[2], (data[1] - 1), data[0], time[0], time[1], 0);
				else
					var date = new Date(data[2], (data[0] - 1), data[1], time[0], time[1], 0);
			}
		}
		else
